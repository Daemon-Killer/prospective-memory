...
const TIME_RULES: TimeRule[] = [
  ...
  {
    pattern: /\b(\+?1h|1 hour|in an hour)\b/i,
    preset: '1h',
    calc: (now) => calculate1Hour(now),
  },
  {
    pattern: /\b(\+?15m|\+15|15 mins?|in 15)\b/i,
    preset: '15m',
...
function applyLingoTemplate(template: string, rest: string): string {
  if (template.includes('$')) {
    return template.replace(/\$/g, rest).replace(/\s+/g, ' ').trim();
  }
...
export function getLingoSuggestions(
  ...
  if (!p) {
    return Array.from(map.entries()).slice(0, limit);
  }
